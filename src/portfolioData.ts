  ...
  projects: [
    {
      ...
      date: "August 2026",
      shortDescription:
        "An end-to-end automated ransomware detection and response workflow using Microsoft Sentinel, Defender for Endpoint, Microsoft Graph, Entra ID, and Azure Logic Apps to detect suspicious mass file renames and automatically contain affected devices and accounts.",
      longDescription:
        "An end-to-end Microsoft security automation project focused on detecting and responding to ransomware activity. The solution uses a Microsoft Sentinel scheduled analytics rule with KQL to identify suspicious high-volume file renames associated with ransomware behavior. When a high-severity incident is created, a Sentinel automation rule triggers an Azure Logic App that extracts affected host and account entities, resolves the Microsoft Defender for Endpoint Device ID, isolates the affected device, starts a full Defender Antivirus scan, resets the compromised user's password, revokes active sign-in sessions, and records the response results in the Sentinel incident. The workflow includes a Microsoft Graph Advanced Hunting fallback for resolving the MDE Device ID when Sentinel does not provide a valid identifier, along with fail-safe validation to prevent isolation against an unknown device. The complete workflow was validated through controlled ransomware simulation and end-to-end testing.",
      technologies: [
        "Microsoft Sentinel",
        ...
        "Microsoft Entra ID",
        "Microsoft Graph",
        "Microsoft Graph Advanced Hunting",
        "Azure Logic Apps",
      ...
      features: [
        "KQL-based ransomware detection using high-volume file rename and extension-change behavior.",
        "Detection logic aligned with MITRE ATT&CK T1486 - Data Encrypted for Impact.",
        "Automatic Microsoft Sentinel incident creation for detected ransomware activity.",
        "Sentinel automation rule triggering an Azure Logic App for automated incident response.",
        "Automatic extraction of affected host and user account entities from Sentinel incidents.",
        "Microsoft Defender for Endpoint device isolation for automated containment.",
        "Automatic full Defender Antivirus scan after successful device isolation.",
        "Microsoft Graph Advanced Hunting fallback to resolve the MDE Device ID by hostname when Sentinel does not provide one.",
        "Automated compromised-user password reset with force-change on next sign-in.",
        "Automatic revocation of active sign-in sessions and refresh tokens.",
        "Incident comments documenting containment, scan, account response, and device-resolution results.",
        "Fail-safe validation that prevents isolation when a valid MDE Device ID cannot be resolved.",
        "Managed Identity authentication and least-privilege permission configuration.",
        "Controlled ransomware simulation used to validate the detection and response workflow.",
        "Complete end-to-end testing from ransomware activity through automated containment and incident documentation.",
      ],
      challenges: [
        "Designing reliable ransomware detection logic based on mass file rename behavior while reducing false positives.",
        "Handling incomplete Sentinel host entity information during automated incident execution.",
        "Resolving the Microsoft Defender for Endpoint Device ID when Sentinel did not provide a valid azureID.",
        "Implementing a safe fallback using Microsoft Graph Advanced Hunting and hostname-based device resolution.",
        "Ensuring containment actions validate the target device identifier before making API requests.",
        "Managing the required Microsoft Defender for Endpoint and Microsoft Graph application permissions for the Logic App Managed Identity.",
        "Configuring Microsoft Entra permissions required for automated password resets and session revocation.",
        "Ensuring the antivirus scan is executed only after successful device isolation.",
        "Handling differences between raw Advanced Hunting telemetry and behavioral ransomware detections surfaced in the Defender portal.",
        "Understanding how repeated ransomware simulation activity can affect the availability of raw FileRenamed telemetry.",
        "Designing the workflow with fail-safe behavior so that unresolved devices are left for manual investigation instead of risking isolation of the wrong endpoint.",
      ],
      learned: [
        "Practical Microsoft Sentinel detection engineering using KQL.",
        "Building ransomware detections around behavioral file-system activity.",
        "Understanding the complete Sentinel incident lifecycle from detection to automated response.",
        "Integrating Microsoft Sentinel with Microsoft Defender for Endpoint for automated containment.",
        "Using Microsoft Graph Advanced Hunting to resolve endpoint information when Sentinel entity data is incomplete.",
        "Automating endpoint isolation and Defender Antivirus scanning.",
        "Automating Entra ID password resets and active-session revocation.",
        "Working with Azure Logic Apps for security orchestration and automated incident response.",
        "Configuring Managed Identity permissions and applying least-privilege principles.",
        "Designing fail-safe security automation with validation and manual-investigation fallbacks.",
        "Understanding the differences between raw security telemetry and higher-level behavioral detections.",
        "Testing security automation using controlled ransomware simulation rather than uncontrolled malicious activity.",
        "Understanding the importance of multiple detection layers instead of relying on a single telemetry source.",
      ],
      featured: true,
      defaultFiles: [
        {
          name: "Microsoft Sentinel Use-Case Implementation Part 1.pdf",
          type: "pdf",
          url: "/projects/Microsoft Sentinel Use-Case Implementation Part 1.pdf",
    ...
    {
      id: "proj17",
      title: "PulseDeck - Cross-Platform Terminal Dashboard",
      company: "Open Source Development",
      date: "August 2026",
      shortDescription:
        "A cross-platform terminal dashboard for CPU, GPU, memory, temperatures, battery, networking, disk space, and running processes — built with Python, psutil, and rich. Supports NVIDIA, AMD, and Intel GPUs on Linux and Windows with automatic layout selection and color-coded metrics.",
      longDescription:
        "PulseDeck is a small Python application that provides a live terminal dashboard for system monitoring. It refreshes approximately every second and automatically selects compact or wide layouts based on terminal size. The dashboard displays CPU utilization with current/max frequency, one-minute load, package temperature, physical-core rows, and logical-thread mapping where exposed. On hybrid systems with discrete NVIDIA GPUs, integrated GPU consumption appears in the CPU panel. NVIDIA metrics include utilization, temperature, VRAM, power draw, and active GPU applications when nvidia-smi is available. AMD and Intel GPU data is sourced through Linux DRM/sysfs when present. The dashboard also shows RAM and swap usage, Wi-Fi and Ethernet interface upload/download rates (with virtual adapters filtered), root filesystem usage, and available NVMe, chipset, Wi-Fi, CPU, and battery sensor readings. The top 16 processes are sorted by normalized CPU usage, showing PID, command, CPU, CPU share, RAM percentage, and RSS. All values are color-coded for quick assessment. The project includes per-user installers for Linux and Windows that create private virtual environments, launcher scripts, and optional KDE autostart integration. Missing drivers, sensors, or permissions are shown as unavailable without breaking the dashboard.",
      technologies: [
        "Python 3.9+",
        "psutil",
        "rich",
        ...
        "KDE Autostart",
        "pyproject.toml",
        "Unicode/ASCII rendering",
      ],
      features: [
        "Live system monitoring with ~1 second refresh rate",
        "Automatic layout selection (compact/wide) based on terminal size",
        "CPU utilization, frequency, load, temperature, and thread mapping",
        "Integrated GPU consumption in CPU panel on hybrid NVIDIA systems",
        "NVIDIA GPU metrics: utilization, temperature, VRAM, power, active apps",
        "AMD/Intel GPU support via Linux DRM/sysfs",
        "RAM and swap usage with color-coded indicators",
        "Network interface upload/download rates with virtual adapter filtering",
        "Root filesystem usage and free space",
        "Sensor readings: NVMe, chipset, Wi-Fi, CPU, battery",
        "Top 16 processes sorted by normalized CPU usage with PID, command, CPU, share, RAM%, RSS",
        "Color-coded usage and temperature values",
        "Graceful handling of missing drivers, sensors, or permissions",
        "Per-user installers for Linux and Windows with private virtual environments",
        "Optional KDE autostart integration on Linux with Konsole",
        "ASCII fallback for terminals without Unicode support",
        "Single snapshot mode with --once flag",
      ],
      challenges: [
        "Handling platform differences between Linux and Windows for hardware metrics",
        "Parsing GPU data from multiple backends (nvidia-smi, DRM/sysfs) with different APIs",
        "Filtering virtual network adapters while showing real interfaces",
        "Computing normalized CPU usage for processes across different CPU architectures",
        "Managing Unicode rendering in terminals and providing ASCII fallback",
        "Creating cross-platform installers with venv and PATH manipulation",
        "Ensuring graceful degradation when sensors or drivers are unavailable",
        "Building a layout system that adapts to terminal size without breaking",
        "Balancing update frequency with system resource usage",
        "Making temperature readings work across diverse hardware and driver configurations",
      ],
      learned: [
        "Deep understanding of cross-platform system monitoring with psutil",
        "Working with NVIDIA's nvidia-smi API for GPU metrics on Linux and Windows",
        "Parsing Linux DRM/sysfs files for AMD and Intel GPU data",
        "Building rich terminal UIs with the rich library",
        "Creating self-contained Python applications with virtual environments",
        "Writing idempotent installers for Linux and Windows",
        "Handling PATH manipulation for user-installed applications",
        "Supporting Unicode and ASCII rendering based on terminal capabilities",
        "Implementing graceful fallbacks for missing hardware or permissions",
        "Designing a modular architecture for platform-specific backends",
        "Creating KDE autostart integration for automatic dashboard launching",
        "Managing process sampling intervals and normalization formulas",
      ],
      featured: true,
    ...
    {
      id: "proj16",
      title: "Android Tablet as a Second Monitor with KDE Plasma & Sunshine",
      company: "Self-Learning & Automation",
      date: "August 2026",
      shortDescription:
        "Transformed a Huawei MatePad T10 into a real extended second monitor for a Fedora/KDE Plasma laptop using a virtual monitor, USB network tethering, and the open-source Sunshine/Moonlight streaming stack — fully automated and wired, no subscriptions.",
      longDescription:
        "A complete hands-on project that turns an old Android tablet into a true extended desktop monitor. KDE's krfb-virtualmonitor creates a virtual output inside KWin, which KDE treats like any physical display, and Sunshine streams that single output over a direct USB (RNDIS) network link to the Moonlight app on the tablet, reaching about 1 ms latency. The virtual screen is positioned to the left of the laptop panel, scaled to 110%, and the primary display is aligned to its computed logical edge to satisfy KDE's no-gaps layout rules. The whole setup is automated: an idempotent bash script starts the virtual monitor, applies position and scaling, and launches Sunshine, while a KDE autostart entry brings everything up at login. The project covers USB networking, KWin virtual outputs, PipeWire/KWin screencast capture, encoder troubleshooting across hybrid Intel/NVIDIA GPUs, and a fully repeatable automation workflow.",
      technologies: [
        ...
        "USB Tethering / RNDIS",
        "kscreen-doctor",
        "KWin ScreenCast / PipeWire",
        "libx264 / FFmpeg",
        "VAAPI / NVENC",
        "Bash Scripting",
        "systemd / KDE Autostart",
        "Fedora Linux",
      ],
      features: [
        "Direct USB network link between laptop and tablet using Android USB tethering (RNDIS) with ~1 ms latency and no router.",
        "KWin virtual monitor created via krfb-virtualmonitor at the tablet's native 1920x1200 resolution.",
        "Virtual screen positioned to the left of the primary display and scaled to 110% for comfortable readability.",
        "Automatic alignment of the primary screen to the virtual screen's computed logical edge to avoid unsupported gaps between displays.",
        "Sunshine configured with KWin screencast capture (capture = kwin) targeting the virtual output by name.",
        "Hardware-encoded alternatives (Intel VAAPI, NVIDIA NVENC, NVIDIA-VAAPI) evaluated and documented, with software libx264 encoder selected as the reliable winner.",
        "Idempotent matepad-on.sh script: starts the virtual monitor, waits for the output, applies position and scale, and starts Sunshine only if needed.",
        "matepad-off.sh script for clean teardown of the streaming server and virtual monitor.",
        "KDE autostart entry making the second screen automatically available at login.",
        "Troubleshooting covered for KWin screencast permissions, encoder incompatibilities, and display layout constraints.",
        "Built-in VNC fallback on the same virtual monitor for quick access without Moonlight.",
      ],
      challenges: [
        "Establishing a reliable direct network link over USB between the laptop and a WiFi-only Android tablet.",
        "Handling KDE's 'Gaps between displays are not supported' constraint, which requires exact edge alignment.",
        "Dealing with the logical width reduction caused by fractional scaling when computing display positions.",
        "Working around kscreen-doctor's rejection of negative coordinates for left-of-primary layouts.",
        "Discovering that every hardware video encoding path failed on a hybrid Intel/NVIDIA GPU laptop (Intel iHD dropped Coffee Lake, Sunshine's CUDA dropped Pascal, NVIDIA-VAAPI exposed no encode profiles).",
        "Debugging a self-matching pkill pattern that terminated the invoking shell during script testing.",
        "Getting KWin screencast permissions correct for capturing a compositor-level virtual output on Wayland.",
        "Keeping the whole workflow repeatable and automated across reboots.",
      ],
      learned: [
        "Practical understanding of KWin virtual outputs and how KDE Plasma manages multi-monitor layouts on Wayland.",
        "Building a low-latency LAN link over USB with Android USB tethering (RNDIS).",
        "Configuring Sunshine with KWin screencast capture and output selection on Wayland.",
        "Troubleshooting GPU video encoders (VAAPI, NVENC) and when to fall back to software encoding.",
        "Automating GUI-bound daemons safely using KDE autostart and idempotent shell scripts.",
        "Programmatic display control with kscreen-doctor, including position, scale, and geometry parsing.",
        "Hands-on debugging of bash process matching and session-environment issues.",
        "Complete workflow: hardware setup, streaming pipeline, display layout, automation, and documentation.",
      ],
      featured: true,
      defaultFiles: [
        {
          name: "Android tablet as second screen.pdf",
          type: "pdf",
          url: "/projects/Android tablet as second screen.pdf",
          category: "Documentation",
        },
      ],
    },
    {
      ...
      date: "August 2026",
      shortDescription:
        "A hands-on Microsoft Sentinel training lab that deploys pre-recorded attack telemetry and walks through threat hunting, custom detection rule creation, threat intelligence integration, and MITRE ATT&CK coverage mapping.",
      longDescription:
        "Hands-on Microsoft Sentinel lab based on the official open-source Azure Sentinel Training Lab, focused on building and testing a cloud security monitoring environment. The lab uses a User-Assigned Managed Identity (UAMI) for Custom Detection Rules, automated Azure Cloud Shell onboarding, and an ARM template deployment that enables Sentinel onboarding, ingests ~20 MB of pre-recorded telemetry, and provisions analytics rules, workbooks, watchlists, and playbooks. It covers KQL-based hunting across CrowdStrike EDR alerts, Palo Alto firewall logs, Okta identity events, and AWS CloudTrail, creating a custom detection rule for multi-tactic compromise on a single device, connecting the Microsoft Defender Threat Intelligence (MDTI) connector and joining indicators against telemetry, and analyzing MITRE ATT&CK rule coverage to identify detection gaps. The lab also includes deployment troubleshooting, playbook role permissions, cost awareness, and cleanup procedures.",
      technologies: [
        "Microsoft Sentinel",
        "Microsoft Azure",
        "Azure Log Analytics",
      ...
      features: [
        "Automated onboarding via Azure Cloud Shell script that creates a User-Assigned Managed Identity and grants Microsoft Graph CustomDetection.ReadWrite.All.",
        "ARM template deployment that enables Sentinel onboarding on the workspace and waits for it to propagate before deploying resources.",
        "Ingestion of ~20 MB of pre-recorded attack telemetry across CrowdStrike, Palo Alto, Okta, and AWS CloudTrail.",
        "Provisioning of analytics rules, workbooks, watchlists, and a playbook (Get-GeoFromIpAndTagIncident) with System-Assigned Managed Identity.",
        "KQL hunting across sources including CrowdStrike multi-tactic device analysis, Palo Alto denied traffic, Okta failed logins, and AWS failed API calls.",
        "Cross-source union query building a chronological timeline of suspicious activity for investigation.",
        "Creation of a scheduled custom detection rule (Lab Stage E1) detecting 3+ MITRE tactics on a single device within a 4-hour window.",
        "Microsoft Defender Threat Intelligence connector setup and indicator ingestion verification via the ThreatIntelIndicators table.",
        "TI correlation using join kind=inner between MDTI indicators and firewall/cloud telemetry.",
        "MITRE ATT&CK coverage analysis, identification of coverage gaps, and mapping of the lab's 10-stage attack chain.",
      ],
      challenges: [
        "Managing the onboarding flow: creating a User-Assigned Managed Identity and waiting for the service principal to be available in Microsoft Graph before granting permissions.",
        "Working around the outdated official ARM template that could not onboard Sentinel to the workspace and required a Sentinel onboarding delay.",
        "Granting the playbook Logic App's managed identity the Microsoft Sentinel Automation Contributor role for automated responses.",
        "Writing KQL queries that accurately extract device names from CrowdStrike DisplayName fields and summarize multi-tactic activity.",
        "Understanding the delay between telemetry ingestion, scheduled rule evaluation, and Sentinel incident creation.",
        "Correlating MDTI indicators against synthetic lab telemetry where real-world indicator matches are expected to be empty.",
        "Managing the permissions required for Log Analytics, Microsoft Sentinel, deployment scripts, and role assignments.",
        "Keeping the lab cost-effective and removing resources and subscription-level configuration after testing.",
      ],
      learned: [
        "Practical understanding of Microsoft Sentinel architecture and its integration with Log Analytics.",
        "Full environment onboarding: UAMI creation, Graph permission grants, and ARM template deployment.",
        "KQL fundamentals for threat hunting across EDR, firewall, identity, and cloud trail data.",
        "Turning a hunting query into a scheduled Microsoft Sentinel analytics rule with entity mapping.",
        "Connecting the Microsoft Defender Threat Intelligence connector and analyzing STIX-formatted indicators.",
        "Correlating threat intelligence indicators against telemetry using KQL join patterns.",
        "Reading MITRE ATT&CK coverage, identifying detection gaps, and mapping an attack chain to techniques.",
        "Azure deployment validation, troubleshooting, permissions management, and security lab cleanup.",
      ],
      featured: true,
      defaultFiles: [
        {
          name: "Microsoft Sentinel Training Lab.pdf",
          type: "pdf",
          url: "/projects/Microsoft Sentinel Training Lab.pdf",
          category: "Documentation",
        },
      ],
    },
