...
import { Terminal, MessageSquare, ArrowRight, ShieldCheck, HardDrive, Key } from 'lucide-react';
...
export function Hero() {
  ...
  useEffect(() => {
    ...
    if (!isDeleting && typedTitle === currentFullTitle) {
      ...
      setIsDeleting(false);
      setCurrentTitleIndex((previous) => (previous + 1) % titles.length);
    }
    ...
    <section id="home" className="relative min-h-screen flex items-center justify-center pt-24 pb-16 overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full z-10 grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
        <div className="lg:col-span-7 flex flex-col items-start gap-6">
          ...
          >
            I&apos;m Muhammad Ayman, a Microsoft security engineer focused on Sentinel, Defender XDR, and automated incident response. I turn cloud telemetry into practical detections and resilient Zero Trust controls.
          </motion.p>
        ...
        <div className="lg:col-span-5 flex justify-center lg:justify-end relative">
          ...
          >
            ...
            <svg className="absolute w-full h-full text-blue-600/10 dark:text-blue-500/10 animate-spin-slow" viewBox="0 0 200 200" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <div className="relative w-52 h-52 sm:w-60 sm:h-60 rounded-full border-2 border-dashed border-blue-500/30 bg-slate-950 dark:bg-slate-900 shadow-2xl flex flex-col items-center justify-center p-6 text-center overflow-hidden">
              <motion.div animate={{ y: [0, -4, 0] }} transition={{ duration: 4, repeat: Infinity, ease: 'easeInOut' }} className="z-10 flex flex-col items-center">
              ...
              <div className="absolute top-8 left-10 text-[10px] font-mono text-emerald-400 flex items-center gap-1 select-none"><ShieldCheck className="w-3 h-3 text-emerald-500" /> PASS</div>
              <div className="absolute bottom-8 right-10 text-[10px] font-mono text-blue-400/80 flex items-center gap-1 select-none"><HardDrive className="w-3 h-3" /> VM_OK</div>
              <div className="absolute bottom-16 left-6 text-[9px] font-mono text-slate-500">SENTINEL_OK</div>
              <div className="absolute top-16 right-6 text-[9px] font-mono text-slate-500">SOAR: AUTO</div>
            ...
            <motion.div animate={{ rotate: 360 }} transition={{ duration: 25, repeat: Infinity, ease: 'linear' }} className="absolute inset-0 w-full h-full pointer-events-none">
              <div className="absolute top-2 left-1/2 -translate-x-1/2 p-2 rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 shadow-md pointer-events-auto"><Key className="w-4 h-4 text-amber-500" /></div>
              <div className="absolute bottom-2 left-1/2 -translate-x-1/2 p-2 rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 shadow-md pointer-events-auto"><ShieldCheck className="w-4 h-4 text-blue-600" /></div>
            </motion.div>
          </motion.div>
        </div>
      </div>
